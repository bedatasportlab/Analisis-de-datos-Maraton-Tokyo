...
Con el principal objetivo de poder analizar posteriormente de una manera más enriquecida los posibles grupos de carrera, se ha decidido crear una nueva variable que determinará el nivel del atleta. Para ello se definirán los siguientes umbrales: 

- Élite: < 2:30:00
- Semi-profesional: 2:30:00 - 3:30:00
- Amateur: > 3:30:00

```{r}

# Aqui habría que filtrar por Hombre o Mujer también...
resultadosTokyo2025 <- resultadosTokyo2025 %>%
  mutate(
    Categoria = case_when(
      tiempo_oficial <= 9000    ~ "Elite",
      tiempo_oficial <= 12600   ~ "Semi-Profesional",
      TRUE                         ~ "Amateur"
    ),
    
    # Convertimos la nueva columna a factor con un orden lógico
    Categoria = factor(Categoria, levels = c("Elite", "Semi-Profesional", "Amateur"))
  )
...
```{r}
rm(edad_disp_forma, edad_stats, moda)
```
...
En este mapa se pueden observar que el país que más participantes tuvo fue Japón, seguramente debido a que dicho maratón se realizó en este país. Otros países que llevaron más de 1000 corredores fueron Estados Unidos, Taiwán, China e Inglaterra.
